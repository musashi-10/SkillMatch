    ...
    if show_apply_form:
        with st.expander("📨 Apply to this position"):
            ...
            if st.button("Submit Application", key=f"asub_{job['id']}"):
                ...
                if res.get("ok"):
                    st.markdown(f'<div class="ok">✅ {res["message"]}</div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="err">⚠️ {res["message"]}</div>', unsafe_allow_html=True)
